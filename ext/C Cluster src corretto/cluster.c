...
{ int i;
  double* rank;
  int* index;
  rank = static_cast<double*>(malloc(n*sizeof(double)));
  if (!rank) return NULL;
  index = static_cast<int*>(malloc(n*sizeof(int)));
  if (!index)
  { free(rank);
    return NULL;
  }
...
{ int i;
  double** data;
  int** mask;
  data = static_cast<double**>(malloc(nrows*sizeof(double*)));
  if(!data) return 0;
  mask = static_cast<int**>(malloc(nrows*sizeof(int*)));
  if(!mask)
  { free(data);
    return 0;
  }
  for (i = 0; i < nrows; i++)
  { data[i] = static_cast<double*>(malloc(ncols*sizeof(double)));
    if(!data[i]) break;
    mask[i] = static_cast<int*>(malloc(ncols*sizeof(int)));
    if(!mask[i])
    { free(data[i]);
      break;
    }
...
{ int i, j, k, i1, k1, l1, its;
  ...
  double scale = 0.0;
  double anorm = 0.0;
  double* rv1 = static_cast<double*>(malloc(n*sizeof(double)));
  if (!rv1) return -1;
  if (m >= n)
...
*/
{
    ...
    int j;
    int error;
    int* index = static_cast<int*>(malloc(ncolumns*sizeof(int)));
    double* temp = static_cast<double*>(malloc(ncolumns*sizeof(double)));
    if (!index || !temp)
    {   if (index) free(index);
        if (temp) free(temp);
        return -1;
...
{ int i;
  ...
  double* tdata1;
  double* tdata2;
  tdata1 = static_cast<double*>(malloc(n*sizeof(double)));
  if(!tdata1) return 0.0; /* Memory allocation error */
  tdata2 = static_cast<double*>(malloc(n*sizeof(double)));
...
{ switch(method)
  { case 'm':
    { const int nelements = (transpose==0) ? nrows : ncolumns;
      double* cache = static_cast<double*>(malloc(nelements*sizeof(double)));
      if (!cache) return 0;
...
{ int i, j, k;
  ...
  int* saved = static_cast<int*>(malloc(nelements*sizeof(int)));
...
{ const int nelements = (transpose==0) ? nrows : ncolumns;
  ...
  *ifound = -1;
  ...
  counts = static_cast<int*>(malloc(nclusters*sizeof(int)));
  if(!counts) return;

  /* Find out if the user specified an initial clustering */
  if (npass<=1) tclusterid = clusterid;
  else
  { tclusterid = static_cast<int*>(malloc(nelements*sizeof(int)));
    if (!tclusterid)
    { free(counts);
      return;
    }
    mapping = static_cast<int*>(malloc(nclusters*sizeof(int)));
    if (!mapping)
    { free(counts);
      free(tclusterid);
      return;
...
{ int i, j, icluster;
  ...
  *ifound = -1;
  ...
  /* We save the clustering solution periodically and check if it reappears */
  saved = static_cast<int*>(malloc(nelements*sizeof(int)));
  if (saved==NULL) return;

  centroids = static_cast<int*>(malloc(nclusters*sizeof(int)));
  if(!centroids)
  { free(saved);
    return;
  }

  errors = static_cast<double*>(malloc(nclusters*sizeof(double)));
  if(!errors)
  { free(saved);
    free(centroids);
    return;
...
{ /* First determine the size of the distance matrix */
  ...
  for (i = 1; i < n; i++)
  { matrix[i] = static_cast<double*>(malloc(i*sizeof(double)));
...
{ int i,j;
  ...
  double* result = static_cast<double*>(malloc(nelements*sizeof(double)));
...
{ int i, j, k;
  ...
  nodeid = static_cast<int*>(malloc(n*sizeof(int)));
  if(!nodeid)
...
{ int i, j;
  ...
  double** newdata;
  int** newmask;
  int* distid = static_cast<int*>(malloc(nelements*sizeof(int)));
  if(!distid) return NULL;
  result = static_cast<Node*>(malloc(nnodes*sizeof(Node)));
  if(!result)
  { free(distid);
    return NULL;
  }
...
{ int i, j, k;
  ...
  int* index;
  Node* result;
  temp = static_cast<double*>(malloc(nnodes*sizeof(double)));
  if(!temp) return NULL;
  index = static_cast<int*>(malloc(nelements*sizeof(int)));
  if(!index)
  { free(temp);
    return NULL;
  }
  vector = static_cast<int*>(malloc(nnodes*sizeof(int)));
  if(!vector)
  { free(index);
    free(temp);
    return NULL;
  }
  result = static_cast<Node*>(malloc(nelements*sizeof(Node)));
  if(!result)
  { free(vector);
    free(index);
    free(temp);
  ...
  result = static_cast<Node*>(realloc(result, nnodes*sizeof(Node)));
...
{ int j;
  ...
  clusterid = static_cast<int*>(malloc(nelements*sizeof(int)));
  if(!clusterid) return NULL;
  result = static_cast<Node*>(malloc((nelements-1)*sizeof(Node)));
  if (!result)
  { free(clusterid);
    return NULL;
  }
...
{ int j;
  ...
  clusterid = static_cast<int*>(malloc(nelements*sizeof(int)));
  if(!clusterid) return NULL;
  number = static_cast<int*>(malloc(nelements*sizeof(int)));
  if(!number)
  { free(clusterid);
    return NULL;
  }
...
{ const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int i, j;
  double* stddata = static_cast<double*>(calloc(nelements,sizeof(double)));
  int** dummymask;
  int ix, iy;
  int* index;
  int iter;
  ...
  if (transpose==0)
  { dummymask = static_cast<int**>(malloc(nygrid*sizeof(int*)));
    for (i = 0; i < nygrid; i++)
    { dummymask[i] = static_cast<int*>(malloc(ndata*sizeof(int)));
      for (j = 0; j < ndata; j++) dummymask[i][j] = 1;
    }
  }
  else
  { dummymask = static_cast<int**>(malloc(ndata*sizeof(int*)));
    for (i = 0; i < ndata; i++)
    { dummymask[i] = static_cast<int*>(malloc(sizeof(int)));
      dummymask[i][0] = 1;
    }
  }
  ...
  { int ixbest = 0;
    ...
    { double closest;
      double** celldatavector = static_cast<double**>(malloc(ndata*sizeof(double*)));
      double radius = maxradius * (1. - ((double)iter)/((double)niter));
      double tau = inittau * (1. - ((double)iter)/((double)niter));
...
{ const int ndata = (transpose==0) ? ncolumns : nrows;
  ...
  if (transpose==0)
  { int** dummymask = static_cast<int**>(malloc(nygrid*sizeof(int*)));
    for (i = 0; i < nygrid; i++)
    { dummymask[i] = static_cast<int*>(malloc(ncolumns*sizeof(int)));
  ...
  { double** celldatavector = static_cast<double**>(malloc(ndata*sizeof(double*)));
    int** dummymask = static_cast<int**>(malloc(nrows*sizeof(int*)));
    int ixbest = 0;
    int iybest = 0;
    for (i = 0; i < nrows; i++)
    { dummymask[i] = static_cast<int*>(malloc(sizeof(int)));
...
{ const int nobjects = (transpose==0) ? nrows : ncolumns;
  ...
  if (lcelldata==0)
  { celldata = static_cast<double***>(malloc(nxgrid*nygrid*ndata*sizeof(double**)));
    for (i = 0; i < nxgrid; i++)
    { celldata[i] = static_cast<double**>(malloc(nygrid*ndata*sizeof(double*)));
      for (j = 0; j < nygrid; j++)
        celldata[i][j] = static_cast<double*>(malloc(ndata*sizeof(double)));
    }
  }
  ...
  { case 'a':
    { /* Find the center */
      ...
      { double distance;
        ...
        int* cmask[2];
        int* count[2];
        count[0] = static_cast<int*>(calloc(ncolumns,sizeof(int)));
        count[1] = static_cast<int*>(calloc(ncolumns,sizeof(int)));
        cdata[0] = static_cast<double*>(calloc(ncolumns,sizeof(double)));
        cdata[1] = static_cast<double*>(calloc(ncolumns,sizeof(double)));
        cmask[0] = static_cast<int*>(malloc(ncolumns*sizeof(int)));
        cmask[1] = static_cast<int*>(malloc(ncolumns*sizeof(int)));
        for (i = 0; i < n1; i++)
        { k = index1[i];
          for (j = 0; j < ncolumns; j++)
            if (mask[k][j] != 0)
      ...
      { double distance;
        int** count = static_cast<int**>(malloc(nrows*sizeof(int*)));
        double** cdata = static_cast<double**>(malloc(nrows*sizeof(double*)));
        int** cmask = static_cast<int**>(malloc(nrows*sizeof(int*)));
        for (i = 0; i < nrows; i++)
        { count[i] = static_cast<int*>(calloc(2,sizeof(int)));
          cdata[i] = static_cast<double*>(calloc(2,sizeof(double)));
          cmask[i] = static_cast<int*>(malloc(2*sizeof(int)));
        }
        for (i = 0; i < n1; i++)
        { k = index1[i];
          for (j = 0; j < nrows; j++)
    ...
    { int i, j, k;
      if (transpose==0)
      { double distance;
        double* temp = static_cast<double*>(malloc(nrows*sizeof(double)));
        double* cdata[2];
        int* cmask[2];
        for (i = 0; i < 2; i++)
        { cdata[i] = static_cast<double*>(malloc(ncolumns*sizeof(double)));
          cmask[i] = static_cast<int*>(malloc(ncolumns*sizeof(int)));
        }
        for (j = 0; j < ncolumns; j++)
        { int count = 0;
          for (k = 0; k < n1; k++)
      ...
      { double distance;
        ...
        int** cmask = static_cast<int**>(malloc(nrows*sizeof(int*)));
        for (i = 0; i < nrows; i++)
        { cdata[i] = static_cast<double*>(malloc(2*sizeof(double)));
          cmask[i] = static_cast<int*>(malloc(2*sizeof(int)));
        }
        for (j = 0; j < nrows; j++)
        { int count = 0;
